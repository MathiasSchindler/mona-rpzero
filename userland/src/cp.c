...
static void usage(void) {
    sys_puts("usage: cp SOURCE... DEST\n");
    sys_puts("       cp SOURCE DEST\n");
    sys_puts("       cp -h|--help\n");
}
...
static int cp_one(const char *src, const char *dst) {
    ...
    if (streq(src, "-")) {
        ...
        if (mode_is_dir(st.st_mode)) {
            sys_puts("cp: omitting directory: ");
            sys_puts(src);
            sys_puts("\n");
            return -1;
        }
        if (!mode_is_reg(st.st_mode)) {
            sys_puts("cp: unsupported file type: ");
            sys_puts(src);
            sys_puts("\n");
            return -1;
        }
        create_mode = (uint32_t)(st.st_mode & 0777u);

        int64_t opened = (int64_t)sys_openat((uint64_t)AT_FDCWD, src, (uint64_t)O_RDONLY, 0);
        if (opened < 0) {
            sys_puts("cp: cannot open: ");
            sys_puts(src);
            sys_puts(" rc=");
            write_i64_dec_local(opened);
            sys_puts("\n");
            return -1;
        }
        in_fd = (uint64_t)opened;
    }

    {
        uint64_t flags = (uint64_t)(O_WRONLY | O_CREAT | O_TRUNC);
        int64_t opened = (int64_t)sys_openat((uint64_t)AT_FDCWD, dst, flags, create_mode);
        if (opened < 0) {
            sys_puts("cp: cannot open dest: ");
            sys_puts(dst);
            sys_puts(" rc=");
            write_i64_dec_local(opened);
            sys_puts("\n");
            if (!streq(src, "-")) (void)sys_close(in_fd);
            return -1;
        }
        out_fd = (uint64_t)opened;
    }
...
int main(int argc, char **argv, char **envp) {
    ...
    if (nsrc > 1) {
        ...
        if (rc < 0 || !mode_is_dir(st.st_mode)) {
            sys_puts("cp: destination is not a directory\n");
            return 1;
        }
        dst_is_dir = 1;
    } else {
    ...
    for (int i = 1; i < 1 + nsrc; i++) {
        ...
        if (dst_is_dir) {
            ...
            if (join_path(dst_path, sizeof(dst_path), dst, bn) != 0) {
                sys_puts("cp: destination path too long\n");
                status = 1;
                continue;
            }
            dst_use = dst_path;
