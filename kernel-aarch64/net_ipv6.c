...
static int ipv6_send_icmp(netif_t *nif,
    ...
    /* Source is always our link-local for now. */
    memcpy_u8(ip6->src, nif->ipv6_ll, 16);
...
static int send_neighbor_solicitation(netif_t *nif, const uint8_t target_ip[16]) {
    ...
    return ipv6_send_icmp(nif, dst_ip, dst_mac, /*hop_limit=*/255, ICMPV6_NEIGHBOR_SOLICIT, 0, body, sizeof(body));
}
...
static int send_neighbor_advertisement(netif_t *nif,
    ...
    return ipv6_send_icmp(nif, dst_ip, dst_mac, /*hop_limit=*/255, ICMPV6_NEIGHBOR_ADVERT, 0, body, sizeof(body));
}
...
static int send_echo_request(netif_t *nif, const uint8_t dst_ip[16], const uint8_t dst_mac[6], uint16_t ident, uint16_t seq) {
    ...
    return ipv6_send_icmp(nif, dst_ip, dst_mac, /*hop_limit=*/64, ICMPV6_ECHO_REQUEST, 0, body, sizeof(body));
}
...
static int send_echo_reply(netif_t *nif,
    ...
    return ipv6_send_icmp(nif, dst_ip, dst_mac, /*hop_limit=*/64, ICMPV6_ECHO_REPLY, 0, echo_body, echo_body_len);
}
...
static void maybe_kick_ping_after_nd_update(const uint8_t updated_ip[16]) {
    ...
    if (!memeq(updated_ip, g_ping_dst_ip, 16)) return;

    uint8_t mac[6];
    if (nd_lookup_mac(g_ping_dst_ip, mac) != 0) return;
...
int net_ipv6_ping6_start(int proc_idx, netif_t *nif, const uint8_t dst_ip[16], uint16_t ident, uint16_t seq) {
    ...
    if (!ipv6_is_linklocal(dst_ip) && !ipv6_is_multicast(dst_ip)) {
        /* Early stack only does on-link link-local. */
    ...
    uint8_t mac[6];
    if (!ipv6_is_multicast(dst_ip) && nd_lookup_mac(dst_ip, mac) == 0) {
        /* Neighbor known: send echo immediately. */
    ...
    g_ping_phase = 1;
    return send_neighbor_solicitation(nif, dst_ip);
}
...
void net_ipv6_input(netif_t *nif, const uint8_t src_mac[6], const uint8_t *pkt, size_t len) {
    ...
    if (icmp->type == ICMPV6_ECHO_REQUEST) {
        ...
        if (!memeq(ip6->dst, nif->ipv6_ll, 16)) return;

        (void)send_echo_reply(nif, ip6->src, src_mac, icmp_body, icmp_body_len);
        return;
    }
